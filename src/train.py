...
class BranchingCompositeTrainer:
    def __init__(
        ...
        self.num_stages = int(cfg.train.num_stages)
        ...
        self.selector_threshold = float(cfg.train.selector_threshold)
        self.grad_clip = float(cfg.train.grad_clip)

        selector_backbone = load_sbert_pooler(self.selector_cfg.sbert_name)
        expert_backbone = load_sbert_pooler(self.expert_cfg.sbert_name)
        
        self.stage_epochs = [int(x) for x in cfg.train.stage_epochs]
        if len(self.stage_epochs) != self.num_stages:
            raise ValueError(
                f"cfg.train.stage_epochs must have exactly {self.num_stages} entries "
                f"(got {len(self.stage_epochs)})"
            )
    ...
    def evaluate_leaves(self, loader, desc="Branch Composite Eval"):
        ...
        for batch in iterator:
            ...
            for (name, pred) in branches_pred:
                ...
                tp, fp, fn = counts(pred, batch["ner_tags"])
    ...
    def evaluate(self):
        ...
        total_metrics = []
        for name in branches_eval_counts.keys():
            tp = branches_eval_counts[name]["tp"]
            fp = branches_eval_counts[name]["fp"]
            fn = branches_eval_counts[name]["fn"]
            eval_f1, eval_p, eval_r = metrics_from_counts(tp, fp, fn)
            tp = branches_dev_counts[name]["tp"]
            fp = branches_dev_counts[name]["fp"]
            fn = branches_dev_counts[name]["fn"]
            dev_f1, dev_p, dev_r = metrics_from_counts(tp, fp, fn)
